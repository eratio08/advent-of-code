...
let rec deliver_package (steps : char list) (pos : int * int) (houses : house list)
  : house list
  =
  match steps with
  | [] -> houses
...
let rec take_turns steps s_pos r_pos houses : house list =
  ...
  | [] -> houses
  | x :: [] ->
    (match create_house x s_pos with
     | None -> houses
     | Some h -> h :: houses)
  | x :: y :: rest ->
    ...
    (match s_house, r_house with
     ...
     | Some x, Some y -> take_turns rest (x.x, x.y) (y.x, y.y) (x :: y :: houses))
;;
